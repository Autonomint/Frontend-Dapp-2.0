...
const CoveredCallTemplate = ({
  ...
  const filteredPriceOptions = selectedTicker === "ETH" ? priceOptions.slice(1) : priceOptions;
      ...
      if (!isBuyMode) {
        ...
        if (stockAssetName !== undefined) {
          const signedData = await refetchCdsDepositSignedData({
            collateralType: getApiAssetName(selectedTicker, isPutOption ? "put" : "call"),
            strikePrice: 0,
            optionFees: "0",
          });
        ...
        if (stockAssetName !== undefined) {
          const signedData = await refetchStockSignedData({
            collateralType: getApiAssetName(ticker, option as "call" | "put"),
            strikePrice: selectedPriceData.strike,
            expiry: selectedDate,
            optionType: option,
          });
                    ...
                    {isBuyMode ? (
                      <>
                        ...
                        <div className="bg-[#f0f4f3] dark:bg-[#161616] border border-grayLight dark:border-grayLight rounded-[10px] p-[14px_16px] mt-4 font-['JetBrains_Mono',monospace] text-[11.5px] leading-[1.6] text-grayLight dark:text-gray-400">
                          {isPutOption && selectedTicker === "LAB" ? (
                            "Your ITM payout is capped at 5% below the strike price. This cap is what makes the discounted premium possible and reflects the collateral committed by put sellers. The cap is however flexible and can be updated to 30% max as per the collateral committed by put sellers"
                          ) : (
                            "Your ITM payout is capped at 5% above the strike price. This cap is what makes the discounted premium possible and reflects the collateral committed by call sellers. The cap is however flexible and can be updated to 30% max as per the collateral committed by call sellers"
                          )}
                        </div>
                      </>
                    ) : (

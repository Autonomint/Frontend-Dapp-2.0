...
const useLastCumulativeRate = (token: string) => {
  ...
  const tokenEnum = token === "cbBTC" ? [AssetName.cbBTC] : token === "krwq" ? [AssetName.KRWQ] : token === "EURC" ? [AssetName.EURC] : token === "HYPE" ? [AssetName.ETH] : undefined;
  const abi = token === "cbBTC" || token === "krwq" || token === "EURC" || token === "HYPE" ? borowCoreABI : borrowingContractAbi;
  const contract = token === "cbBTC" || token === "krwq" || token === "EURC" ? borrowCoreAddress : borrowingContractAddress;

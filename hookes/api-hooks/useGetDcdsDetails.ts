  ...
  const updatePagedData = () => {
    if (dcdsPositionList) {
      ...
      setPagedDcdsPositionList(() => {
        ...
        const data = [...sortedData.filter((item) => item.collateralType === "ETH"), ...sortedData.filter((item) => item.collateralType === "cbBTC" || item.collateralType === "krwq" || item.collateralType === "EURC" || item.collateralType === "HYPE")]

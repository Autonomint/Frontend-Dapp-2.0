...
export function hasDaysPassed(timestamp: number, daysInSeconds: number): boolean {
  const timestampInMs = timestamp * 1000; // Convert input timestamp from seconds to milliseconds
  const durationInMs = daysInSeconds * 1000; // Convert days (in seconds) to milliseconds
  const now = Date.now(); // Current time in milliseconds
